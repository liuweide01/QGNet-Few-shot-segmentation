...
class PFENet(nn.Module):
    def __init__(self, layers=50, classes=2, zoom_factor=8, \
        ...
        pretrained=True, sync_bn=False, shot=1, ppm_scales=[60, 30, 15, 8], vgg=False):
        ...
        if layers == 50:
            resnet_unsuper = models.resnet50(pretrained=pretrained,deep_base=False,unsuper=True)
        ...
        self.layer0_unsuper = nn.Sequential(resnet_unsuper.conv1, resnet_unsuper.bn1, resnet_unsuper.relu, resnet_unsuper.maxpool)
        self.layer1_unsuper, self.layer2_unsuper, self.layer3_unsuper, self.layer4_unsuper = resnet_unsuper.layer1, resnet_unsuper.layer2,\
                                                                                             resnet_unsuper.layer3, resnet_unsuper.layer4
        ...
        for n, m in self.layer3_unsuper.named_modules():
            if 'conv2' in n:
                m.dilation, m.padding, m.stride = (2, 2), (2, 2), (1, 1)
            elif 'downsample.0' in n:
                m.stride = (1, 1)
        for n, m in self.layer4_unsuper.named_modules():
            if 'conv2' in n:
                m.dilation, m.padding, m.stride = (4, 4), (4, 4), (1, 1)
            elif 'downsample.0' in n:
                m.stride = (1, 1)
        ...
        for bin in self.pyramid_bins:
            self.init_merge.append(nn.Sequential(
                nn.Conv2d(reduce_dim*2 +1+1, reduce_dim, kernel_size=1, padding=0, bias=False),
                nn.ReLU(inplace=True),
            ))                      
        ...
        self.down_learn_super = nn.Sequential(
            nn.Conv2d(2048, reduce_dim,  kernel_size=3, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.5)
        )
        self.down_learn_unsuper = nn.Sequential(
            nn.Conv2d(2048, reduce_dim,  kernel_size=3, padding=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.5)
        )
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None):
        ...
        with torch.no_grad():
            ...
            query_feat_0_unsuper = self.layer0_unsuper(x)
            query_feat_1_unsuper = self.layer1_unsuper(query_feat_0_unsuper)
            query_feat_2_unsuper = self.layer2_unsuper(query_feat_1_unsuper)
            query_feat_3_unsuper = self.layer3_unsuper(query_feat_2_unsuper)
            query_feat_4_unsuper = self.layer4_unsuper(query_feat_3_unsuper)

        query_sup = self.down_learn_super(query_feat_4)
        query_unsup = self.down_learn_unsuper(query_feat_4_unsuper)
        cosine_eps = 1e-7
        q = query_sup
        s = query_unsup
        ...
        unsup_query = similarity.view(bsize, 1, sp_sz, sp_sz)
        unsup_query = F.interpolate(unsup_query, size=(query_feat_3.size()[2], query_feat_3.size()[3]), mode='bilinear',
                                   align_corners=True)
        ...
        for idx, tmp_bin in enumerate(self.pyramid_bins):
            ...
            unsup_query_bin = F.interpolate(unsup_query, size=(bin, bin), mode='bilinear', align_corners=True)
            merge_feat_bin = torch.cat([query_feat_bin, supp_feat_bin,corr_mask_bin,unsup_query_bin], 1)
...
if __name__=='__main__':
    ...
    model = PFENet(layers=50, classes=2, zoom_factor=8, \
        ...
        pretrained=True, shot=1, ppm_scales=[60, 30, 15, 8])
    print(model)
