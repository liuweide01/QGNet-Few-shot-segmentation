...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None):    
    ...
    # for key in sub_class_file_list.keys():
    #     list_key = sub_class_file_list[key]
    #
    #     with open(os.path.join( str(key) + '.txt'), 'w' )as filehandle:
    #         for listitem in list_key:
    #             filehandle.write('%s\n' % listitem[1])
...
if __name__=='__main__':
    ...
    val_list = './val.txt'
    train_list = 'voc_sbd_merge_noduplicate.txt'
    data_root = '/home/wdliu/VOC/VOC2012EX/VOC2012_SEG_AUG'

    val_data = SemData(split=3, shot=1, data_root=data_root, \
                            data_list=train_list, mode='train', \
